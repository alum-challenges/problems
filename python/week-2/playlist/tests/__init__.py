...
def test_one_song_output():
    ...
    output = """
1. Soothsayer by Backethead
"""

    check50.run("python3 playlist.py").stdin("7", prompt=True).stdin(
        "Soothsayer", prompt=True
    ).stdin("Buckethead", prompt=True).stdout(output, output, regex=True).exit()
...
"""
    ...
    check50.run("python3 playlist.py").stdin("7", prompt=True).stdin(
        "Soothsayer", prompt=True
    ).stdin("Buckethead", prompt=True).stdin(
        "Everybody Wants to Rule the World", prompt=True
    ).stdin(
        "Tears for Fears", prompt=True
    ).stdin(
        "Everything in Its Right Place", prompt=True
    ).stdin(
        "Radiohead", prompt=True
    ).stdin(
        "Visa fran Utanmyra", prompt=True
    ).stdin(
        "Jan Johansson", prompt=True
    ).stdin(
        "Road to Nowhere", prompt=True
    ).stdin(
        "Talking Heads", prompt=True
    ).stdin(
        "Orbaum", prompt=True
    ).stdin(
        "Grip on the News", prompt=True
    ).stdin(
        "Fool me twice.", prompt=True
    ).stdin(
        "kuiper", prompt=True
    ).stdout(
