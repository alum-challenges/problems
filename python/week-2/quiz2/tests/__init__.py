...
def pytest_question_one():
    ...
    check50.run(
        "pytest --tb=no --color=no -q testing.py -k 'test_question1 and not test_question10 and not test_question11'"
    ).stdout("1 passed").exit(0)
...
@check50.check(exists)
def pytest_question_eleven():
    """question ten answered correctly"""
    check50.run("pytest --tb=no --color=no -q testing.py -k 'test_question11'").stdout(
        "1 passed"
    ).exit(0)
